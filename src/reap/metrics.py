...
def pairwise_mean_squared_l2(vectors: torch.Tensor) -> torch.Tensor:
    """Return pairwise mean squared L2 distance between row vectors.

    ``vectors`` has shape ``[num_items, num_observations]``.  The result at
    ``[i, j]`` is ``mean((vectors[i] - vectors[j]) ** 2)``.  The Gram-matrix
    formulation avoids materialising a ``[N, N, T]`` difference tensor.
    """
    if vectors.ndim != 2:
        raise ValueError(
            f"vectors must have shape [num_items, num_observations], got "
    ...
    mean_square = vectors.square().mean(dim=1)
    gram = (vectors @ vectors.T) / vectors.shape[1]
    distance = mean_square[:, None] + mean_square[None, :] - 2.0 * gram
