...
class MoETransformerObserver(BaseTransformerObserver):
    ...
    def _initialize_state(self, output: torch.Tensor, num_experts: int):
        ...
        if not self.hook_config.record_pruning_metrics_only:
            ...
            # Exact E_x[(g_i(x) - g_j(x))^2] geometry of the dispatched top-k
            # router weights used by Fisher clustering. Keep the legacy cosine
            # statistic above for reproducibility with existing experiments.
            layer_state["router_weight_l2_distance"] = OnlineStatsTracker(
                shape=(num_experts, num_experts),
                count_shape=1,
                device=device,
                dtype=torch.float32,
    ...
    def _hook_factory(self, module: nn.Module, layer_number: int) -> callable:
        ...
        def _hook_fn(module, args, output):
            ...
            if not self.hook_config.record_pruning_metrics_only:
                ...
                dense_router_weights = torch.softmax(
                    pruning_batch.router_logits, dim=1, dtype=torch.float32
                )
                selected_router_weights = torch.gather(
                    dense_router_weights,
                    1,
                    pruning_batch.selected_experts,
                )
                if self.hook_config.renormalize_router_weights:
                    selected_router_weights = selected_router_weights / (
                        selected_router_weights.sum(dim=1, keepdim=True)
                        + torch.finfo(selected_router_weights.dtype).eps
                    )
                router_weights = torch.zeros_like(dense_router_weights).scatter_(
                    1,
                    pruning_batch.selected_experts,
                    selected_router_weights,
                )
                router_weight_l2 = pairwise_mean_squared_l2(
                    router_weights.transpose(0, 1)
                ).to(device="cpu")
                self.state[layer_number]["router_weight_l2_distance"].update(
                    router_weight_l2, pruning_batch.num_tokens
                )
                del (
                    dense_router_weights,
                    selected_router_weights,
