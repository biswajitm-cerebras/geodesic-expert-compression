...
class ClusterArgs:
    ...
    fisher_distance_weight: float = field(
        default=1.0,
        metadata={
            "help": (
                "Weight of the median-normalized local Fisher-Rao distance when "
                "--expert-sim=fisher_geodesic."
            )
        },
    )
    router_distance_weight: float = field(
        default=0.25,
        metadata={
            "help": (
                "Weight of router geometry when --expert-sim=fisher_geodesic. "
                "New caches use mean squared router-probability distance; legacy "
                "caches fall back to router-logit cosine distance."
            )
        },
    )
    activation_distance_weight: float = field(
        default=0.25,
        metadata={
            "help": (
                "Weight of the persisted activation distance when "
                "--expert-sim=fisher_geodesic."
            )
        },
    )
    fisher_sketch_size: int = field(
        default=32,
        metadata={
            "help": (
                "Number of input coordinates sampled per expert projection for "
                "the Fisher distance. Use <=0 for the exact local metric (large)."
            )
        },
    )
    fisher_distance_cache_path: str | None = field(
        default=None,
        metadata={
            "help": (
                "Optional cache path for Fisher-geodesic clustering matrices. "
                "Defaults to <results_dir>/fisher_geodesic_distances.pt."
            )
        },
    )
    fisher_distance_overwrite: bool = field(
        default=False,
        metadata={
            "help": "Recompute Fisher-geodesic clustering matrices if cached."
        },
    )
...
class MergeArgs:
    ...
    fisher_num_batches: int = field(
        ...
        metadata={
            "help": (
                "Number of calibration batches used to estimate the per-expert "
                "diagonal Fisher for the 'v27_fisher_geodesic' merge method."
            )
        },
    )
    fisher_cache_path: str | None = field(
        default=None,
        metadata={
            "help": (
                "Path to cache/load the per-expert Fisher tensors for "
                "'v27_fisher_geodesic'. Defaults to <results_dir>/v27_expert_fisher.pt."
            )
        },
    )
    fisher_overwrite: bool = field(
        default=False,
        metadata={
            "help": (
                "Recompute and overwrite the cached per-expert Fisher even if a "
                "cache file already exists."
            )
        },
    )
    fisher_router_weight_source: str = field(
        default="none",
        metadata={
            "help": (
                "Scalar expert importance used to condition Fisher clustering and "
                "the Fisher barycenter. 'reap' uses router-weighted activation norm; "
                "'router' uses summed gate mass; 'frequency' uses top-k counts; "
                "'none' preserves the original v27 behavior."
            ),
            "choices": ["none", "frequency", "router", "reap"],
        },
    )
