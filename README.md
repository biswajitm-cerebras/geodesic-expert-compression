...
#### Router-weighted Fisher expert geometry (experimental)

The repository also provides an experimental merge pipeline that changes both
cluster construction and the merged expert center:

1. collect a diagonal Fisher metric for each expert projection;
2. construct an endpoint-average local Fisher--Rao distance, optionally using a
    deterministic coordinate sketch;
3. condition each expert metric with corpus router importance (`reap`, `router`,
    `frequency`, or `none`);
4. median-normalize and combine Fisher, dispatched-router, and activation
    distances;
5. form clusters with average-linkage agglomerative clustering; and
6. create a new expert center with the existing Fisher-weighted merge, optionally
    refined using Gauss--Legendre path-integrated Fisher (`GL_ITERS>0`).

Run the paper-scale SLURM recipe with:

```bash
sbatch sbatch_fisher_geodesic_cluster.sh
```

The principal CLI controls are `--expert_sim fisher_geodesic`,
`--fisher_distance_weight`, `--router_distance_weight`,
`--activation_distance_weight`, `--fisher_sketch_size`, and
`--fisher_router_weight_source`. The endpoint Fisher and combined distance
matrices are cached separately; set `--fisher_overwrite true` or
`--fisher_distance_overwrite true` to refresh them. A newly generated observer
cache records mean-squared distance between actual top-k dispatched router
weights. Older caches remain usable through the router-logit cosine fallback.

**Scope.** Pairwise clustering uses a local endpoint-average diagonal-Fisher
approximation, not an exact solution of every pairwise Fisher--Rao geodesic.
Gauss--Legendre path integration is applied to the cluster-center merge. The
router-conditioned Fisher is a mean-field approximation based on routing-aware
gradients and corpus REAP importance, rather than the exact conditional quantity
for every token and expert pair.
