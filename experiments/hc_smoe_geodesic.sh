...
run_math=${10:-true}
# wildbench
run_wildbench=${11:-true}
singleton_super_experts=${12:-false}
singleton_outlier_experts=${13:-false}

echo "Running HC-SMoE + Fisher-Geodesic merge: $model_name, dataset: $dataset_name, compression ratio: $compression_ratio, seed ${seed}, super expert singletons: ${singleton_super_experts}, outlier expert singletons: ${singleton_outlier_experts}"

cluster_description="hc_smoe_geodesic"
output_file_name="observations_1024_cosine-seed_${seed}.pt"
if [[ "${singleton_super_experts}" == "true" ]]; then
    merged_model_dir_name="hc_smoe_geodesic-super_expert_singletons-seed_${seed}_${compression_ratio}"
elif [[ "${singleton_outlier_experts}" == "true" ]]; then
    merged_model_dir_name="hc_smoe_geodesic-outlier_expert_singletons-seed_${seed}_${compression_ratio}"
else
    merged_model_dir_name="hc_smoe_geodesic-seed_${seed}_${compression_ratio}"
fi
...
echo "Finished HC-SMoE + Fisher-Geodesic merge for model: ${model_name}, dataset: ${dataset_name}, compression ratio: ${compression_ratio}"