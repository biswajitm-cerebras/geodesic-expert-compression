...
#SBATCH --no-requeue
#SBATCH --output=logs/v27glew_slice_%j.out
#SBATCH --error=logs/v27glew_slice_%j.err
...
export ROUTER_COMPACT=slice              # <-- compact router to num_clusters rows (REAP-style)
...
VARIANT_TAG="v27glew_slice"
MERGED_MODEL_DIR="${VARIANT_TAG}-paper-seed_${SEED}_${COMPRESSION_RATIO}_it${GL_ITERS}_n${GL_NODES}"

echo "========================================================"
echo "  Variant:          ${VARIANT_TAG}  (FISHER_ELEMENTWISE=1, ROUTER_COMPACT=slice)"
...
python src/reap/main.py \
    ...
    --plot_clusters       true \
    2>&1 | tee "logs/v27glew_slice_merge_${SLURM_JOB_ID}.log"
